...
const RouteModel = require("../static_gtfs_models/Route");
const StopModel = require("../static_gtfs_models/Stop");
const TripModel = require("../static_gtfs_models/Trip");
const StopTimeModel = require("../static_gtfs_models/StopTime");
...
async function importGTFS(feedId, feedUrl) {
  try {
    ...
    const tempDir = path.join(__dirname, "../../tmp", feedId);
    ...
    await parseAndInsert(tempDir, feedId, feedUrl);
...
async function insertRoutes(file, feedId) {
  ...
  await RouteModel.deleteMany({ feedId });
  if (docs.length) await RouteModel.insertMany(docs);
...
async function insertCalendarDates(file, feedId) {
  ...
  console.log(`🗓️ Inserted ${docs.length} calendar_dates`);
}
